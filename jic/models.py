...
class Model(nn.Module):
    ...
    def __call__(self, batch, test):
        ...
        full_lattice = jnp.reshape(
            full_lattice, (
                full_lattice.shape[0], full_lattice.shape[1],
                full_lattice.shape[2] * full_lattice.shape[3]
            )
        )
        full_lattice = jax.lax.stop_gradient(full_lattice)
        regrouped_lattice = self.regrouping_head(full_lattice)
        encoded_lattice = self.Downstream_Encoder(
            regrouped_lattice, batch["num_frames"]
        )
        encoded_lattice = self.dropout_layer(
            encoded_lattice, deterministic=test
        )
        encoded_lattice = self.second_downstream_Encoder(
            encoded_lattice, batch["num_frames"]
        )
